...
class LetterSet(private[antimirov] val array: Array[Char]) { lhs =>
  ...
  override def equals(that: Any): Boolean =
    that match {
      case ls: LetterSet => util.Arrays.equals(array, ls.array)
      case _ => false
    }
...
object Diff {
  final case class Left[A](a: A) extends Diff(a) {
    override def isLeft: Boolean = true
    override def isBoth: Boolean = false
    override def isRight: Boolean = false
  }
  final case class Both[A](a: A) extends Diff(a) {
    override def isLeft: Boolean = false
    override def isBoth: Boolean = true
    override def isRight: Boolean = false
  }
  final case class Right[A](a: A) extends Diff(a) {
    override def isLeft: Boolean = false
    override def isBoth: Boolean = false
